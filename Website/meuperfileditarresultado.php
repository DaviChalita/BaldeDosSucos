...
<h3 style="margin: 16px 16px 16px 16px;">Cadastre-se</h3>
<?php

if(!isset ($_POST["cadastroLogin"]) == true){
?>
<center><b>Nenhuma informação foi recebida para alterar os dados.</b></p>
...
<?php
	...
	if ($conecta = @mysql_connect($local_server, $usuario_server, $senha_server)){
		if ($db = @mysql_select_db($banco_dados, $conecta)){
			...
			$sql = "Update cliente set strSenha='".$senha."', strTelefone='".$telefone."', strCEP='".$cep."', strEndereco='".$endereco."', strNumero='".$numero."', strComplemento='".$complemento."', strBairro='".$bairro."', strCidade='".$cidade."', strEstado='".$estado."'  where strLogin = '".$login."'";
			if ($query = @mysql_query($sql)){
?>
				<center>
				</br>
				<img src="Imagens/cadastrosucesso.png"><p>
				<b>Seus dados foram alterados com sucesso, <?php echo "$login"; ?>!</b><p>
...
?>
				...
				<b>Erro ao editar perfil.</b><p>
				Erro ao inserir informações no banco de dados.<p>
				<input type="submit" value="Voltar à tela de edição de perfil" onclick="history.go(-1);" /><p>
				</center>
<?php
			}
			mysql_close($conecta);
